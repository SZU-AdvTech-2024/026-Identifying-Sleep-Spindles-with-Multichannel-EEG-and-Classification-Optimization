...
class Filter_based_and_thresholding:
    ...
    def get_epochs(self):
        ...
        epochs = mne.Epochs(raw,events,event_id=1,tmin=0,tmax=validation_windowsize,baseline=(0, 0),
                           preload=True)
        # psds,freq = psd_array_multitaper(epochs,fmin=l_freq,
        #                                 fmax=h_freq,
        #                                 tmin=0,tmax=validation_windowsize,
        #                                 low_bias=True,)
        psds = epochs.compute_psd(
            method='multitaper', fmin=l_freq, fmax=h_freq
        )
        freqs = psds.freqs
        psds = 10 * np.log10(psds.get_data())  # 转换为 dB

        # psds = 10 * np.log10(psds.get_data())
        self.epochs = epochs
        self.psds = psds
        self.freq = freqs
